...
// Displays options and returns the user's choice index (0-based)
    ...
    void runTest(int sock, const vector<MCQ>& questions) {
        ...
        for (const auto& q : questions) {
            ...
            if (ans == q.correctAnswer) {
                ...
                feedback << "❌ Wrong. Correct answer is: " << q.correctAnswer << "\n";
            }
...
for (const auto& subject : subjects) {
    cerr << "[DEBUG] Found subject: " << subject << endl;
}

// Step 2: Get user's subject choice
int subjIndex = getChoiceFromList(sock, subjects, "AVAILABLE SUBJECTS");
string subject = subjects[subjIndex];  // Now you have the selected subject
        ...
        quiz.runTest(sock, questions); // assume runQuiz accepts sock to send questions and receive answers
